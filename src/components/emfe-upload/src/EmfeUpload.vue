<template>
  <div class="emfe-upload" :class="uploadName">
    <template v-if="type === 'icon'">
      ...
      <input v-show="!src" class="emfe-upload-file" :class="fileName" :disabled="disabled || !canUpload" type="file" accept="image/*" @change="change" ref="upload">
    ...
    <template v-if="type === 'plus'">
      ...
      <input v-show="!src" class="emfe-upload-file" :class="fileName" :disabled="disabled || !canUpload" type="file" accept="image/*" @change="change" ref="uploadPlus">
      ...
      if (/^image/.test(file.type)) {
        ...
        reader.onloadend = () => {
          ...
          if (reader.result.length <= (100 * 1024)) {
            this.postHandle(upload(reader.result, this.uploadJpeg));
          } else {
            img.onload = () => {
              const data = this.compress(img, Orientation);
              this.postHandle(upload(data, this.uploadJpeg));
            };
          }
        };
      }
      ...
      if (((width * height) / 1000000) > 1) {
        ...
        for (let i = 0; i < count; i++) {
          for (let j = 0; j < count; j++) {
            const nwRatio = i * nw * ratio;
            const nhRatio = j * nh * ratio;
            tctx.drawImage(img, nwRatio, nhRatio, nw * ratio, nh * ratio, 0, 0, nw, nh);
